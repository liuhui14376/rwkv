...
#         v
# +-----------------------------+            +-----------------------+
# | conv2d_features (Sequential) |            | senet (SE Module)     |
# | Conv2d -> BatchNorm -> ReLU  |            | Squeeze -> Excitation |
# +-----------------------------+            +-----------------------+
#         |
#         v
# +------------------------------------------------------------+
# |               encoder_block (function)                     |
# |  1. Feature extraction using conv2d_features               |
# |  2. Positional Embedding (get_position_embedding)          |
# |  3. Spectral pixel data embedding                          |
# |  4. Call local_trans_pixel (RWKV)                          |
# +------------------------------------------------------------+
#         |
#         v
# +--------------------------------------------+
# |      local_trans_pixel (RWKV Model)        |
# +--------------------------------------------+
#         |
#         v
# +---------------------------------------+     +-------------------------------------+
# | RWKV_TimeMix (Time-Based Mixing)      |     | RWKV_ChannelMix (Channel Mixing)    |
# +---------------------------------------+     +-------------------------------------+
#         |                                      |
#         v                                      v
# +----------------------+              +----------------------+
...
# |    (Attention)       |              |    (Channel Mixing)  |
# +----------------------+              +----------------------+
#         |                                      |
#         v                                      v
# +----------------------+              +----------------------+
# |   Apply attention    |              |   Apply channel mix  |
# |   mechanism          |              |                      |
# +----------------------+              +----------------------+
#         |                                      |
#         v                                      v
# +--------------------------------------------+
# |               Center Extraction            |
# |    Extract center pixel for aggregation    |
# +--------------------------------------------+
#         |
#         v
# +---------------------------------------------------+
# |   classifier_mlp (Final Classifier)               |
# |   Linear -> BatchNorm -> Dropout -> ReLU -> Linear|
# +---------------------------------------------------+
#         |
#         v
# +-----------------------+
...
# +-----------------------+
#
# SQSFormer 类 作为主模型调用入口，负责图像输入处理、特征提取和分类。
#
# 先通过 conv2d_features 提取初步卷积特征。
# 然后通过 senet 进行特征增强。
# encoder_block 负责将特征送入 RWKV 模块处理。
# encoder_block 进行图像特征的编码，并处理位置嵌入。然后调用 local_trans_pixel 来进行 RWKV 模型处理。
#
# RWKV 模块 分为两个部分： RWKV_TimeMix 处理时序上的混合操作，RWKV_ChannelMix 负责通道上的混合操作。
#
# RWKV_TimeMix 和 RWKV_ChannelMix 通过 key, value, receptance 机制来进行注意力和通道混合，分别计算时序和通道信息。
#
# Center Extraction 负责提取编码后图像的中心信息，并返回给分类器。
#
# classifier_mlp 作为最终的分类器，进行全连接层操作，输出预测结果。