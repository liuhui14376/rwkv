...
class HSIDataLoader(object):
    def __init__(self, param) -> None:
        ...
        # self.data_sign = self.data_param.get('data_sign', 'Honghu')
    ...
    def load_raw_data(self):   #数据分类
        ...
        data_path = '%s/%s/%s.mat' % (self.data_path_prefix, self.data_sign, self.data_file)

        # data_path = '%s/%s/%s_split.mat' % (self.data_path_prefix, self.data_sign, self.data_file)
        data_path = '../data/Indian/Honghu.mat'
    ...
    def get_train_test_patches(self, X, y, test_size=0.2):  # 增加test_size参数
        h, w, c = X.shape
        # 给 X 做 padding
        print("****************************")
        print("*****************************")
        ...
        valid_indices = [(r, c) for r in range(margin, zeroPaddedX.shape[0] - margin)
                         for c in range(margin, zeroPaddedX.shape[1] - margin)
                         if y[r - margin, c - margin] > 0]

        # 使用 train_test_split 对索引进行随机划分
        train_indices, test_indices = train_test_split(valid_indices, test_size=test_size, random_state=42)

        trainX_index2pos = {i: pos for i, pos in enumerate(train_indices)}
        testX_index2pos = {i: pos for i, pos in enumerate(test_indices)}
        all_index2pos = {i: pos for i, pos in enumerate(valid_indices)}
    ...
    def prepare_data(self):
        ...
        base_img, labels, train_index2pos, test_index2pos, all_index2pos, margin, patch_size \
            = self.get_train_test_patches(norm_data, self.labels, test_size=0.2)
